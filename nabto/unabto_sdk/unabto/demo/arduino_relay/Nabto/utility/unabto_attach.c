...
text stName(nabto_state state)
{
    switch (state) {
        ...
        case NABTO_AS_WAIT_DNS_FALLBACK : return "WAIT_DNS_FALLBACK";
...
void handle_as_wait_bs(void) {
    ...
    if (nmc.nabtoMainSetup.forceDnsFallback) {
        if (!nmc.context.useDnsFallback) {
            SET_CTX_STATE_STAMP(NABTO_AS_WAIT_DNS_FALLBACK, 0);
            return;
        }
    }
#endif
    ...
    if(++nmc.context.counter > 6) {
        ...
        SET_CTX_STATE_STAMP(NABTO_AS_WAIT_DNS_FALLBACK, INTERVAL_ERROR_RETRY_BASE);
        return;
    }
...
void handle_as_wait_dns_fallback(void)
{
#if NABTO_ENABLE_DNS_FALLBACK
    SET_CTX_STATE_STAMP(NABTO_AS_WAIT_DNS_FALLBACK, EXP_WAIT(INTERVAL_DNS_FALLBACK, nmc.context.counter));
    if(++nmc.context.counter > 6) {
    ...
    make_dns_fallback();
#else
    NABTO_LOG_INFO(("Dns fallback disabled. Retrying from beginning"));
    SET_CTX_STATE_STAMP(NABTO_AS_IDLE, INTERVAL_ERROR_RETRY_BASE);
#endif
}
...
void nabto_attach_time_event(void)
{
    ...
    case NABTO_AS_WAIT_DNS_FALLBACK: handle_as_wait_dns_fallback(); break;
...
void nabto_network_changed(void) {
    ...
    case NABTO_AS_WAIT_DNS_FALLBACK:
