...
void nabto_rendezvous_end(nabto_connect* con) {
...
#if NABTO_ENABLE_EXTENDED_RENDEZVOUS_MULTIPLE_SOCKETS
    {
        ...
        if (!rcs->socketsHasBeenClosed) {
            int i;
            for (i = 0; i < rcs->socketsOpened; i++) {
                if (con->socket == rcs->sockets[i]) {
                    // do not close this socket!
                } else {
                    NABTO_LOG_TRACE(("Rendezvous end, closing socket %i", rcs->sockets[i]));
                    nabto_close_socket(&rcs->sockets[i]);
                }
            }
            rcs->socketsHasBeenClosed = true;
        }
    }
#endif
}
