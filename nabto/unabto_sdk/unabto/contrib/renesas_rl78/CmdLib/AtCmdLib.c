        ...
        uint16_t udpClientPort)
{
    ...
    if (ATLIBGS_INVALID_CID != cid) {
        ...
        if (ATLIBGS_CON_UDP_SERVER == conType) {
            ...
            sprintf(cmd, "%c%c" _F8_ "%s:" "%u" ":", ATLIBGS_ESC_CHAR, 'U',
                    cid, pUdpClientIP, udpClientPort);
            ...
            //sprintf(cmd, "%c%c" _F8_ "%s:" "%u" ":" "%s", ATLIBGS_ESC_CHAR, 'Y',
            //          cid, pUdpClientIP, udpClientPort, digits);
        } else {
...
ATLIBGS_MSG_ID_E AtLibGs_BulkUDP(
        uint8_t cid,
        const void *txBuf,
        uint16_t dataLen,
        const char *pUdpClientIP,
...
void AtLib_GSLinkSendValue( int8_t *pTag, uint8_t cid, int32_t value)
{
    ...
    itoa(value, (char*)cData, 10);
    
    sprintf ((char*)&(dataBuf[0]),"%s%c%s", pTag,':',cData); 
    command_length = strlen((char const*)dataBuf);      /* Get command length */
...
#endif
    sprintf ((char*)&(dataBuf[0]),"%s%c%.1fF", pTag,':',gTemp_F);   
    command_length = strlen((char const*)dataBuf);      /* Get command length */
    AtLib_ConvertNumberTo4DigitASCII(command_length, cDataLen);
