...
// TODO
    ...
    if (tcb->streamState >= ST_ESTABLISHED) {
        ...
        {
            bool newAck = (tcb->ackSent != tcb->recvTop) ||
                (tcb->lastSentAdvertisedWindow != unabto_stream_advertised_window_size(tcb));
            if (newAck ||
                windowHasOpened ||
                (tcb->cfg.enableWSRF && (tcb->ackWSRFcount < tcb->cfg.maxRetrans) && nabtoIsStampPassed(&tcb->ackStamp))) {
...
static bool unabto_stream_create_sack_pairs(struct nabto_stream_s* stream, struct nabto_stream_sack_data* sackData)
{
    ...
    for (i = tcb->recvTop; i < (tcb->recvNext + tcb->cfg.recvWinSize); i++) {
