...
int main(int argc, char *argv[]) {
   ...
   n = 0;
   CParser7Ctor(&cparser);            /* explicitely call the Ctor */
   CParser7Init(&cparser);              /* take initial transition */
   while ((ch = fgetc(f)) != EOF) {
       ...
       StateTableDispatch((StateTable *)&cparser, sig);  /* upcast */
       ++n; 
   }
   fclose(f);
   ...
   printf("file %s contains %ld comment characters, "
          "%ld total.\n", 
          argv[1], CParser7GetCommentCtr(&cparser), n);
