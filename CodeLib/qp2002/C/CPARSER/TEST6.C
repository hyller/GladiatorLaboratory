...
int main(int argc, char *argv[]) {
   ...
   n = 0;
   CParser6Ctor(&cparser);            /* explicitely call the Ctor */
   QHsmInit((QHsm *)&cparser, 0);       /* take initial transition */
   while ((ch = fgetc(f)) != EOF) {
       ...
       QHsmDispatch((QHsm *)&cparser, e);              /* dispatch */
       ++n; 
   }
   fclose(f);
   ...
   printf("file %s contains %ld comment characters, "
          "%ld comment blocks, "
          "%ld total.\n", 
          argv[1], CParser6GetCommentCtr(&cparser), 
          CParser6GetCommBlkCtr(&cparser), n);
