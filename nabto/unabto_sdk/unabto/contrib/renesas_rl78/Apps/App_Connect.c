...
ATLIBGS_MSG_ID_E App_Connect(ATLIBGS_WEB_PROV_SETTINGS *wp)
{
    ...
    while (1) {
        ...
        if (wp->dhcp_enable) {
            ...
            rxMsgId = AtLibGs_IPSet("192.168.70.22", "255.255.255.0", "192.168.70.1");
            if (rxMsgId != ATLIBGS_MSG_ID_OK) {
		...
		if ((uint8_t)wp->security == (uint8_t)ATLIBGS_SMWEP) {
		...
		} else if ((uint8_t)wp->security != (uint8_t)ATLIBGS_SMOPEN) {
